-- ══════════════════════════════════════════════════════════════════════════
-- 20260712120012_sacrament_engine.sql   (idempotent; safe to re-run)
-- BACK-PORT of churchos-saas-sacraments.sql — the agentic sacrament engine —
-- into the canonical migration chain. The engine was applied to LIVE
-- out-of-band and existed ONLY as a root script: no chain migration created
-- sacrament_requests / sacrament_events (0008 already *references* them
-- defensively), so a fresh `supabase db push` could not rebuild the live
-- schema (structural audit 2026-08-13 §1 — same failure class as the
-- Yayamove no-GRANT chain). Prod already has all of this: applying it there
-- is a no-op by construction (IF NOT EXISTS / OR REPLACE / DROP IF EXISTS
-- before CREATE).
--
-- Content is churchos-saas-sacraments.sql (now archive/sql/) with exactly
-- three deviations, each required for a FRESH build:
--   (a) §0's five `drop trigger … on public.baptism_*` statements are wrapped
--       in a to_regclass() guard: DROP TRIGGER IF EXISTS still ERRORS when the
--       TABLE is absent, and no chain migration ever creates the baptism_*
--       tables (they existed on prod only via the retired baptism-specific
--       script, and this section already dropped them there).
--   (b) §6b adds explicit table GRANTs — this family's known rebuild failure
--       class is grants applied out-of-band that a fresh chain silently loses.
--       Client roles get the RLS-gated DML verbs only, deliberately NO
--       TRUNCATE (RLS cannot gate it; 0011 revoked it from client roles on
--       prod and flipped the default privileges to match).
--   (c) this header.
--
-- Ordering: runs after 0001 (rate_check, touch_updated_at, auth_parish_id,
-- availability_slots, the four *_records register books) and 0007
-- (is_untrusted_client_write, client_ip). 0008's sacrament-policy drop is
-- existence-guarded, so it no-ops on a fresh build and this migration's own
-- §6 enforces the same parish-local (RA 10173) end-state either way.
-- ══════════════════════════════════════════════════════════════════════════
...
-- Guarded for the fresh-build path: DROP TRIGGER IF EXISTS errors when the
-- TABLE does not exist, and the chain never creates the baptism_* tables.
do $$
begin
  if to_regclass('public.baptism_requests') is not null then
    execute 'drop trigger if exists trg_normalize_baptism on public.baptism_requests';
    execute 'drop trigger if exists trg_guard_baptism_approval on public.baptism_requests';
    execute 'drop trigger if exists trg_touch_baptism on public.baptism_requests';
    execute 'drop trigger if exists trg_log_baptism_event on public.baptism_requests';
  end if;
  if to_regclass('public.baptism_events') is not null then
    execute 'drop trigger if exists trg_baptism_events_no_mutate on public.baptism_events';
  end if;
end $$;
...
-- ── 6b. Explicit table grants (rebuild-safety; additive no-op on prod) ────────
-- On hosted Supabase the platform's default privileges cover these, but the chain
-- must not depend on grant state applied out-of-band (this family's known rebuild
-- failure class). Client roles get exactly the RLS-gated DML verbs — deliberately
-- NO TRUNCATE, which RLS cannot gate (0011 revoked it from the client roles and
-- flipped the default privileges to match). RLS above stays the enforcement layer:
-- anon has an INSERT policy only; authenticated is parish-scoped by policy.
grant select, insert on public.sacrament_requests to anon;
grant select, insert, update, delete on public.sacrament_requests to authenticated;
grant select, insert, update, delete on public.sacrament_events to authenticated;
grant all on public.sacrament_requests to service_role;
grant all on public.sacrament_events to service_role;
